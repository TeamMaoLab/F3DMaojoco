...
>
> 本文档描述的是 2025-09 模块化重构时的设计。其中涉及的**"向后兼容性设计"章节（main_app.py / control_panels.py 兼容包装器）已过时** —— 这两个包装器文件已于 2026-07-14 删除（`gui/__init__.py` 中对应的 `legacy_main` / `legacy_create_control_panel` 导出也已移除）。
>
> 阅读本文档时，"模块拆分前后对比"和"向后兼容性设计"两节请当作**历史记录**，当前实际 gui/ 结构以 `gui/__init__.py` 的模块 docstring 为准。
