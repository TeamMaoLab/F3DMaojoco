...
>
> **本纲要是在功能开发过程中边写边改的，存在大量与实际代码不符的内容，阅读时请以下方声明为准。**
>
> ### 已知的不符点
>
> 1. **本文档提到的下列模块/文件在代码中并不存在**（属于曾经规划或已废弃的设计）：
>    - `core/binding_manager.py`、`core/binding_tester.py`、`core/keyframe_manager.py`、`core/motion_player.py`、`core/motion_interpolation.py`
>    - `gui/main_app.py`、`gui/control_panels.py`（曾经的兼容性包装器，已于 2026-07-14 删除）
>    - `gui/binding_wizard.py`、`gui/pose_editor.py`、`gui/motion_editor.py`（实际功能由 `gui/motion_editor_tab.py` 承担）
>    - `gui/timeline_widget.py`、`gui/keyframe_widget.py`、`gui/motion_controls.py`
>    - `tools/binding_test.py`、`tools/data_analyzer.py`
>    - 数据文件 `gui_config.json`、`saved_poses.json`、`motion_sequences.json`（实际是 `data/poses.json`、`data/motions.json`）
> 2. **`pose_manager.py` 实际位于 `gui/`，不在 `core/`**。
> 3. **完成度数字不可信**：本文档上方"当前状态"和历史进度段落里出现的 95% / 85% 等百分比是开发过程中的乐观估计，与实现存在出入，尤其**动作编辑器的姿态插值、时间轴、关键帧拖拽等核心功能并未实现**（当前播放只是关键帧之间的瞬时跳变）。详见 `motion-editor-development-plan.md`。
> 4. 目录树、mermaid 数据流图、配置文件说明等段落仍保留了上述不存在模块的引用，待后续逐步清理。
>
> 真实代码结构请参考 `gui/__init__.py` 的模块 docstring 和 `core/__init__.py` 的导出。
...
### 🚀 当前状态（2026-07-14 重启核对）
- **已完成且可用的功能**：
  - ✅ 8 自由度关节实时控制（关节映射、滑块、精细控制、腿部分组）
  - ✅ 专业级 3D 渲染（通用轨道相机 + MuJoCo 物理引擎）
  - ✅ 姿态管理系统（保存/加载/删除/导入导出，持久化到 `data/poses.json`）
  - ✅ 动作序列的数据模型与持久化（CRUD、播放/暂停/停止/速度/循环）
- **未完成的功能**（文档其他段落曾声称"基本完成"，实际并未实现）：
  - ❌ 动作播放的姿态插值（当前是关键帧瞬时跳变，无 linear/smooth 插值）
  - ❌ 可视化时间轴、关键帧拖拽编辑
  - ❌ 撤销/重做
- **项目状态**：⚠️ **核心控制可用，动作编辑器为半成品** —— 关节控制与姿态管理可正常使用，动作编辑器只能做关键帧序列的"瞬时切换"播放，插值与时间轴 UI 待实现。
...
> ⚠️ 见本文档顶部的"文档状态声明"。下方历史进度段落中的完成度百分比（如 85%、各阶段 100%）为开发过程中的乐观估计，不代表当前真实完成度。动作编辑器的插值、时间轴等功能实际未实现。

**项目整体完成度**: 见顶部声明（关节控制/姿态管理/渲染可用；动作编辑器为半成品）
**项目状态**: ⚠️ 核心控制可用，动作编辑器待完善
